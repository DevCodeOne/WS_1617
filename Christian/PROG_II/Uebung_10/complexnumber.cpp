...
m_serialnr(create_object_serial()) {
	if (output)
		std::cout << __PRETTY_FUNCTION__ << std::endl;
}

complexnumber::complexnumber(const complexnumber &c) : complexnumber(c.m_real, c.m_imag) {
	if (output)
		std::cout << __PRETTY_FUNCTION__ << std::endl;
}
...
// anders herum ist effizienter, weil so viele Kopien angelegt werden,
// besser + auf -= aufbauen
//complexnumber &complexnumber::operator+=(const complexnumber &right) {
...
complexnumber &complexnumber::operator*=(const complexnumber &right) {
	this->m_real = this->m_real * right.m_real - this->m_imag * right.m_imag;
	this->m_imag = this->m_real * right.m_imag - this->m_imag * right.m_real;
	return *this;
}

complexnumber &complexnumber::operator/=(const complexnumber &right) {
	double div = std::pow(right.m_real, 2) + std::pow(right.imag(), 2);
	this->m_real = (this->m_real * right.m_real - this->imag() * right.imag()) / div;
	this->m_imag = (this->m_imag * right.real() - this->m_real * right.imag()) / div;
	return *this;
}
